...
def main(base_path, graph_sizes):
    aggregated_results = []
    aggregated_errors = []
    
    for algorithm in os.listdir(base_path):
        if not algorithm == "dijkstra":
            continue
            ...
            for graph_size in graph_sizes:
                ...
                for model_type in os.listdir(model_output_dirs):
                    if not "_" in model_type:
                        ...
                        model_name = "Llama_" + "".join(model_type.split("_")[-2:])
                    else:
                    ...
                    for reasoning_strategy in os.listdir(reasoning_strategies_dir):
                        ...
                        for file_ in os.listdir(model_dir):
                            if (model_name in file_ or model_name.split("_")[0] in file_) and "_inference" in file_.lower():
                                if "gpt" in model_dir and "IO_no_chat" in model_dir: print(model_dir)
                                try:
                                    metrics_dict, results, error_analysis = evaluate(os.path.join(model_dir, file_), algorithm, graph_size)
                                    if "gpt" in model_dir and "IO_no_chat" in model_dir: print(metrics_dict)
                                ...
                                # except:
                                #     print(f"Algorithm: {algorithm}, Graph Size: {graph_size}, Model: {model_name}, Reasoning Strategy: {reasoning_strategy} has error")
    ...
    train_utils.save_results_to_csv(aggregated_results, os.path.join(evaluation_output_dir, 'aggregated_results.csv'))
    train_utils.save_results_to_excel(aggregated_results, os.path.join(evaluation_output_dir, 'aggregated_results.xlsx'))
    train_utils.save_results_to_latex(aggregated_results, os.path.join(evaluation_output_dir, 'aggregated_results.tex'))  
    
    train_utils.save_results_to_csv(aggregated_errors, os.path.join(evaluation_output_dir, 'aggregated_errors.csv'))
    train_utils.save_results_to_excel(aggregated_errors, os.path.join(evaluation_output_dir, 'aggregated_errors.xlsx'))
    train_utils.save_results_to_latex(aggregated_errors, os.path.join(evaluation_output_dir, 'aggregated_errors.tex'))                             
                

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Read configuration JSON file')
    parser.add_argument("--base_path", default="/local2/ataylor2/algorithmic_reasoning", type=str)
    ...
    main(args.base_path, args.graph_sizes)